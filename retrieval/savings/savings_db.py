...
def is_savings_there(institution_code, isa, regular_saver, regular_saver_frequency_period, regular_saver_frequency_type, regular_saver_min_amt, regular_saver_max_amt, bonus, bonus_frequency_period, bonus_frequency_type, online, branch, variability, savings_period, min_amt, max_amt, gross_percent, aer_percent, child, interest_paid):
	return mortgagecomparison_db.is_item_there(sql_is_savings_there,(institution_code, isa, regular_saver, regular_saver_frequency_period, regular_saver_frequency_type, regular_saver_min_amt, regular_saver_max_amt, bonus, bonus_frequency_period, bonus_frequency_type, online, branch, variability, savings_period, min_amt, max_amt, gross_percent, aer_percent, child, interest_paid))
...
def update_savings_jrnl(date,savings_id,url_id,institution_code):
	...
	# insert if not there.
	if not mortgagecomparison_db.is_item_there(sql_is_savings_jrnl_there,(savings_id,)):
		mortgagecomparison_db.run_sql(sql_insert_savings_jrnl,(date,savings_id,date,url_id))
	else:
		savings_jrnl_id = mortgagecomparison_db.get_item_id(sql_get_savings_jrnl_there,(savings_id,))
		mortgagecomparison_db.run_sql(sql_update_savings_jrnl_retrieval,(date,url_id,savings_id))


# Gets the mortgage id that matches the mortgage details.
def get_savings_id(institution_code, isa, regular_saver, regular_saver_frequency_period, regular_saver_frequency_type, regular_saver_min_amt, regular_saver_max_amt, bonus, bonus_frequency_period, bonus_frequency_type, online, branch, variability, savings_period, min_amt, max_amt, gross_percent, aer_percent, child, interest_paid):
	return mortgagecomparison_db.get_item_id(sql_get_savings_there,(institution_code, isa, regular_saver, regular_saver_frequency_period, regular_saver_frequency_type, regular_saver_min_amt, regular_saver_max_amt, bonus, bonus_frequency_period, bonus_frequency_type, online, branch, variability, savings_period, min_amt, max_amt, gross_percent, aer_percent, child, interest_paid))

def delete_savings_not_current(institution_code,date,forcedelete,logger):
	logger = logging.getLogger('retrieve')
	logger.info('In delete_savings_not_current')
	mortgagecomparison_db.cursor.execute(sql_count_savings_jrnl_institution_there,(institution_code,))
	row = mortgagecomparison_db.cursor.fetchone()
	count = int(row[0])
	deletecount = 0
	logger.info('There are %s savings rows currently',count)
	mortgagecomparison_db.cursor.execute(sql_get_savings_not_retrieved_on_date,(institution_code,date))
	for row in mortgagecomparison_db.cursor.fetchall():
		...
		mortgagecomparison_db.run_sql(sql_update_savings_jrnl_delete,(date, savings_id))
		deletecount += 1
	...
	if count > 0 and deletecount == count and forcedelete == False:
		mortgagecomparison_utils.record_error('Would have deleted all savings rows for ' + institution_code + ', check logs',logger,mortgagecomparion_db.db_connection,mortgagecomparison_db.cursor)
		exit()
	return
